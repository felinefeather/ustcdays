...
impl MapSystem {
    ...
    pub fn travel(&mut self, to: &str, time_system: &mut crate::time_system::TimeSystem) -> Result<(), String> {
        let current_map = self.maps.get(&self.current_location).ok_or("当前地图不存在")?;
